...
def processRestInspectCSV():
    ...
    rest_inspect_df = rest_inspect_df.loc[
        rest_inspect_df['inspect_dt'] >= cutoff]  # dataframe of recent inspections in Pittsburgh
    rest_inspect_df['inspect_dt'] = rest_inspect_df['inspect_dt'].transform(lambda x: x.date())
    rest_inspect_df = rest_inspect_df.loc[rest_inspect_df['category_cd'].isin(
    ...
    rest_inspect_df = rest_inspect_df.drop(columns=['encounter', 'id', 'placard_st', 'bus_st_date', 'category_cd',
                                                    ...
                                                    'reispt_cd', 'reispt_dt', 'num', 'street', 'status', 'zip', 'city',
                                                    'state'])
    rest_inspect_df.sort_values(by=['inspect_dt', 'street_address'], ascending=False,
                                inplace=True)  # sort dataframe so the most recent inspection of every restarurant is on top
    print(f'rest_inspect_df Before drop {rest_inspect_df.shape}')
    rest_inspect_df.drop_duplicates(subset=['street_address'], inplace=True,
                                    keep='first')  # drop duplicates, and keep only the most recent inspection of a restaurant
...
def processSoupDf():
    ...
    sp_df = sp_df.loc[sp_df[
                          'Post Date'] >= cutoff]  # dataframe of most recent inspections in Pittsburgh (everything else is in the CSV)
...
def addSoupRows(rest_inspect_df):
    ...
    for index, row in soup_df.iterrows():
        rest_inspect_df = rest_inspect_df.append(
            {'facility_name': row['facility_name'], 'street_address': row['street_address'],
             'inspect_dt': row['Post Date'], 'purpose': row['purpose'], 'placard_desc': row['placard_desc'],
             'description': 'Unknown'},
            ignore_index=True)
...
def processGeoFoodFacsCSV():
    ...
    geo_df = geo_df.loc[geo_df['category_cd'].isin(
        category_cd_list)]  # dataframe for food facilities in Pittsburgh of relevant categories
    geo_df['street_address'] = geo_df['num'] + ' ' + geo_df['street'] + ' Pittsburgh, PA'

    geo_df = geo_df.drop(
        columns=['id', 'num', 'street', 'city', 'state', 'municipal', 'category_cd', 'description', 'p_code', 'fdo',
                 'bus_st_date', 'bus_cl_date', 'seat_count', 'noroom', 'sq_feet', 'status', 'placard_st', 'status',
                 'zip'])
...
def mergeGEOCSV(rest_inspect_df, geo_valid_df):
    merge_df = pd.merge(rest_inspect_df, geo_valid_df, on=['street_address', 'facility_name'], how='left',
                        validate="1:1")
...
# Columns to add to Merge_DF
# COVID = Has it been inspected for COVID? Did it fail? Did it Pass?
...
def checkCOVID(str):
    if str.find('COVID') >= 0 or str.find('face coverings') >= 0 or str.find('social distancing') >= 0 or str.find(
            'distancing') >= 0:
        return True
    else:
        return False
...
def addMapColorCodeCols(df):
    ...
    for index, row in df.iterrows():
        ...
        hovText = f"Name: {row['facility_name']} <br>Address: {index} <br>Most Recent Inspection Date:{row['inspect_dt']} <br>Inspection Status:{row['placard_desc']} <br>Inspection Purpose:{row['purpose']}"
...
def update_figure(chosen_type, chosen_status, input_address):
    df_sub = merge_df[(merge_df['description'].isin(chosen_type)) &
                      (merge_df['placard_desc'].isin(
                          chosen_status))]  # updates figure depending on restuarant type and status
    ...
    long = coordinates[0]
    lat = coordinates[1]
    df_sub = df_sub.append({'longitude': long, 'latitude': lat, 'Colors': '#0326D1', 'Hover Text': 'YOU ARE HERE'},
                           ignore_index=True)
...
def main():
    # Get Graphs and Analysis from Kyle
    if input('Do you want to see some cool graphs? (yes or no) ').upper() == 'YES':
        st.getStats()
    app.run_server(debug=False)

# #--------------------------------------------------------------   
# Run server and Get Graphs    
